...
class GFEloqua extends GFFeedAddOn {
	...
	public function init(){
		...
		add_action('gform_enqueue_scripts', array($this, 'frontend_scripts'));
	}
	...
	public function process_feed( $feed, $entry, $form ){
		...
		if (array_key_exists('elqCustomerGUID', $_POST))  {
			$visitor = $this->eloqua->get_visitor($_POST['elqCustomerGUID']);
			if (empty($visitor) || !isset($visitor->contactId))
				$form_submission->submittedByContactId = NULL;
				
			$form_submission->submittedByContactId = $visitor->contactId;
		} else {
			$form_submission->submittedByContactId = NULL;
		}
	...
	public function frontend_scripts() {
		wp_enqueue_script(
			'gfeloqua-frontend', 
			$this->get_base_url() . '/assets/js/gfeloqua-frontend.js',
			array( 'jquery' ),
			$this->_version,
			true
		);
